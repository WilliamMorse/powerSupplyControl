...
class PowerSupply():
    ...
    def writeToPort(self, bytearrayCommandToWrite):
        ...
        temp = re.search(r'\d+\.\d', out) # look for a number in temp
        if bool(temp): # if we find a numbers in the string, 
            out = float(temp.group()) # put them together and cast them as a float.
        else: # if we don't have a number or a null, then we have gotten a text command back or an error!
            if out == 'CC' or out == 'CV': # numeric commands
                return(out)
            elif out == 'Syntax Error':
                ...
                print('partial message receved: %s' % out)
                #raise Exception('partial message receved: %s' % out)
        return(out)
    ...
    def voltage(self, setVoltage = 'None'):
        ...
        if setVoltage != 'None': # if we pass a value other than none, try to set the voltage
            ...
            self.writeToPort(bytearray('VOLT ' + voltage, 'utf-8')) #format and write to the port
            out = None
    ...
    def current(self, setCurrent = 'None'):
        ...
        if setCurrent != 'None': # if we pass a value other than none, try to set the current    
            ...
            self.writeToPort(bytearray('CURR ' + current, 'utf-8')) #format and write to the port
            out = None
